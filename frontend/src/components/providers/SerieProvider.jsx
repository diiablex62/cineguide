...
export default function SerieProvider({ children }) {
  ...
  const loadSerieDetails = useCallback(
    async (id) => {
      if (!id) return;
      ...
      try {
        ...
        if (
          !saisonsData ||
          !Array.isArray(saisonsData) ||
          saisonsData.length === 0
        ) {
          try {
            saisonsData = await getSaisonsBySerie(id);
          } catch (saisonError) {
            console.warn(
              `Impossible de récupérer les saisons pour la série ${id}:`,
              saisonError
            );
            saisonsData = [];
          }
        ...
        if (Array.isArray(saisonsData) && saisonsData.length > 0) {
          ...
          const saisonPromises = saisonsData.map(async (saison) => {
            ...
            try {
              ...
              console.error(
                `Erreur lors du chargement des épisodes de la saison ${saisonNumero}:`,
                episodeError
              );
              return {
                ...saisonObj,
          ...
          try {
            ...
            console.error(
              "Erreur lors du traitement parallèle des saisons:",
              parallelError
            );
          }
        }
        ...
        const normalizedSerie = {
          ...
          saisons:
            saisonsCompletes.length > 0
              ? saisonsCompletes
              : Array.isArray(serieDetails.saisons)
      ...
      if (setter === setAlreadySeenStates) {
        setSeriesSeen((prev) => {
          if (prev.some((serie) => String(serie.id) === String(id))) {
            ...
            const serieToAdd = series.find(
              (serie) => String(serie.id) === String(id)
            );
  ...
  const filterGender = useCallback(
    (value) => {
      if (!value) return allSeries();
  ...
  const filterNotSeen = useCallback(() => {
    setFilteredSeries(
      series.filter(
        (serie) =>
          !seriesSeen.some(
            (seenSerie) => String(seenSerie.id) === String(serie.id)
          )
      )
    );
  }, [series, seriesSeen]);
  ...
  const searchSerie = useCallback(
    (value) => {
      ...
      setFilteredSeries(
        series.filter(
          (serie) =>
            serie.titre && serie.titre.toLowerCase().includes(searchTerm)
        )
      );
    },
    [series]

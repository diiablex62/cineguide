...
export default function ActualiteDetail() {
  const [likes, setLikes] = useState(null); // Initialisez à null

  useEffect(() => {
    const storedLikes = localStorage.getItem("j'aime");
    if (storedLikes) {
      setLikes(parseInt(storedLikes, 10)); // Récupère et convertit la valeur en entier
    } else {
      setLikes(0); // Définit à 0 si aucune valeur n'est trouvée
    }
  }, []);

  const handleLike = () => {
    const newLikes = (likes || 0) + 1; // Ajoute 1 même si likes est null
    setLikes(newLikes);
    localStorage.setItem("j'aime", newLikes); // Stocke la valeur dans le localStorage
  };
          ...
          <div className='mt-4'>
            ...
            <div className='flex justify-end mt-2'>
              <div className='flex items-center gap-2'>
                <button
                  onClick={handleLike} // Ajout de l'événement onClick
                ...
                {likes !== null && ( // Affiche uniquement si likes est défini
